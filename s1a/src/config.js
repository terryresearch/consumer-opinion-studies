...
const CONFIG = {
  ...
  /* Completion. `completionUrl` set = participants are returned automatically
     ...
     the code embedded in the URL. Same Prolific completion link as fr1. */
...
/* ---------------------------------------------------------------------
   STIMULUS
   The article is IDENTICAL in both conditions. Control shows nothing
   beneath it; the mixed condition adds both verdict cards.
   --------------------------------------------------------------------- */
const STIMULUS = {
  kicker: "Skincare",

  /* Product photo, shown identically in both conditions.
  ...
  /* The two magazines. Fictional, so no real publication is depicted
     ...
     would. Because which magazine delivers which verdict is randomised,
     tying colour to the publication counterbalances colour against verdict
     automatically — no extra random factor, and neither hue is
     valence-loaded (no green = good, red = bad). */
  sources: [
