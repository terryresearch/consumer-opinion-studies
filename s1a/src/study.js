/* =====================================================================
   Consumer Opinion Studies — Study 1a: Facial Roller Study (2 cells)
...
(function () {
  ...
  async function startSession() {
    ...
    if (prolific) { S.pid = prolific; S.pidSource = "prolific"; }
      ...
      const key = CONFIG.studyId + "_pid";
      let anon = sessionStorage.getItem(key);
      if (!anon) { anon = "anon-" + makeUid(); sessionStorage.setItem(key, anon); }
    ...
    const a = await rpc("assign", {
      ...
      p_pid: S.pid,
      p_session: sessionStorage.getItem(CONFIG.studyId + "_sid") || (function () {
        const v = makeUid(); sessionStorage.setItem(CONFIG.studyId + "_sid", v); return v;
      })(),
      p_meta: meta
    });
    ...
    /* `label_order` is the strictly alternating counterbalance handed back
       by assign(): in the mixed cell it alternates which verdict is shown
       first. Control has no counterbalance and shows no verdicts. */
  ...
  /* Resolve the two publications onto their roles. Each carries its own
     ...
     colour against verdict for free — nameSwap already randomises which
  ...
  /* =====================================================================
     5. SUBMIT — the answers, plus the realised stimulus assignment.
     ===================================================================== */
  function buildPayload() {
    return Object.assign({}, S.answers);
  }
